...
const validationSchema = Yup.object().shape({
  ...
  type: Yup.string().oneOf(['fixed', 'variable']).required('El tipo es requerido'),
  variations: Yup.array().of(
    Yup.object().shape({
      cost: Yup.number().min(0, 'El costo debe ser mayor o igual a 0').required('El costo es requerido'),
      currency_id: Yup.number().min(1, 'Debe seleccionar una moneda').required('La moneda es requerida'),
      description: Yup.string().required('La descripción es requerida'),
      measure: Yup.string().required('La medida es requerida'),
      product_variations: Yup.array().of(
        Yup.object().shape({
          id: Yup.number().required(),
          required_supplies: Yup.number().min(0, 'La cantidad requerida debe ser mayor o igual a 0').required('La cantidad requerida es necesaria'),
        })
      ),
    })
...
const updateSupplyVariationProductVariations = async (supplyVariationId: string | number, productVariations: ProductVariationRelation[]) => {
...
const updateProductSupplies = async (supplyId: string, productIds: number[]) => {
  // Primero, eliminamos todas las relaciones existentes
  const { error: deleteError } = await supabase
            ...
            try {
                ...
                if (id) {
                    ...
                    if (supply.shop_id !== user.shopId) {
                        throw new Error(
                            'No tienes permiso para editar este suministro'
                        )
                    }

                    // Cargar variaciones existentes del suministro
                    const variations = await fetchExistingVariationsBySupplyId(
                        id
                    )
        ...
        try {
            ...
            if (values.id) {
                ...
                for (const variation of values.variations) {
                  await updateSupplyVariationProductVariations(variation.id, variation.product_variations)
                ...
                const variations = await createSupplyVariations(values.variations.map(v => ({
                  ...v,
                  supply_id: id
                })))
                                            ...
                                            <Card key={index} className="mb-4">
                                                <div className="pt-6">
                                                    ...
                                                    <div className="space-y-4">
                                                        ...
                                                        <div>
                                                            <label
                                                                htmlFor={`variations.${index}.measure`}
                                                            >
                                                                Medida
                                                            </label>
                                                            <Field
                                                        ...
                                                        <div className="space-y-4 mt-2">
                                                          {products.filter(p => values.products.includes(p.id)).map((product, key) => (
                                                          <div key={key}>
                                                              {product.variations.map(pv => (
                                                                <div key={pv.id} className="ml-4 mt-2">
                                                                  <div className="flex items-center space-x-2">
                                                                    <Checkbox
                                                                      id={`pv-${pv.id}-${index}`}
                                                                      checked={values.variations[index].product_variations.some(v => v.id === pv.id)}
                                                                      onChange={(checked) => {
                                                                        const newProductVariations = checked
                                                                          ? [...values.variations[index].product_variations, { id: pv.id, required_supplies: 0 }]
                                                                          : values.variations[index].product_variations.filter(v => v.id !== pv.id)

                                                                          console.log(newProductVariations)
                                                                        setFieldValue(`variations.${index}.product_variations`, newProductVariations)
                                                                      }}
                                                                    />
                                                                    <label htmlFor={`pv-${pv.id}-${index}`}>{pv.name}</label>
                                                                  </div>
                                                                  {values.variations[index].product_variations.some(v => v.id === pv.id) && (
                                                                    <div className="mt-2 ml-6">
                                                                      <label htmlFor={`pv-${pv.id}-${index}-required`}>Cantidad Requerida para la Variación</label>
                                                                      <Field
                                                                        name={`variations.${index}.product_variations[${values.variations[index].product_variations.findIndex(v => v.id === pv.id)}].required_supplies`}
                                                                        type="number"
                                                                        as={Input}
                                                                        className="mt-1 w-24"
                                                                      />
