...
export const supabaseService = {
    getProductsWithVariationsAndSupplies: async () => {
        ...
        const { data: productVariations, error: variationsError } =
            await supabase.from('product_variations').select('*').order('id')
        ...
        const productsWithVariations = products.map((product) => {
            ...
            const variationsWithSupplies = variations.map((variation) => {
                ...
                const suppliesForVariation = supplyVariations
                    .filter((supplyVariation) =>
                        supplyIds.includes(supplyVariation.id)
                    )
                    .map((supplyVariation) => ({
                        ...supplyVariation,
                        supply:
                            supplies.find(
                                (supply) =>
                                    supply.id === supplyVariation.supply_id
                            ) || null,
                    }))
            ...
            const productSuppliesForProduct = productSupplies
                ...
                .map(
                    (ps) =>
                        supplies.find((supply) => supply.id === ps.supply_id) ||
                        null
                )
    ...
    getSupplies: async (): Promise<Supply[]> => {
        ...
        const currencyIds = variations.map((variation) => variation.currency_id)
            ...
            const supplyVariations = variations
                ...
                .map((variation) => ({
                    ...
                    currency_id: currencies.find(
                        (currency) => currency.id === variation.currency_id
                    ),
                }))
    ...
    getSupplyVariationsBySupplies: async (
        ...
        const { data: supplyVariations, error: variationsError } =
            await supabase
                .from('supply_variation')
                .select('*')
                .in('supply_id', supplyIds)
...
export const updateProductSupplyAssociation = async (
  ...
  const { error } = await supabase
    ...
    .upsert({ product_id: productId, supply_id: supplyId, is_associated: isAssociated })
  if (error) throw error
}
...
export const updateVariationSupplyAssociation = async (
  ...
  const { error } = await supabase
    .from('variation_supplies')
    .upsert({ variation_id: variationId, supply_id: supplyId, is_associated: isAssociated })
  if (error) throw error
}
...
export async function createProductSupply(productId: number, supplyId: number, requiredSupplies: number, shopId: number) {
  const { data, error } = await supabase
    .from('product_supplies')
    .insert([
      { product_id: productId, supply_id: supplyId, required_supplies: requiredSupplies, shop_id: shopId }
    ])
